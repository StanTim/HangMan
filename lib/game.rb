class Game
  ...
  def letters_to_guess
    result = @letters.map do |letter|
        if @user_guesses.include?(letter) or @user_guesses.include?(normalize_letter(letter))
          letter
        else
          nil
        end
  ...
  # Метод заменяет в полученных буквах Ё, Й на Е, И соответственно.
  ...
  # Метод заменяет массив на массив с заменёнными Ё, И.
  def normalized_letters(array)
    array.map { |letter| normalize_letter(letter) }
  end
